...
p1 = (245,171,179)
r1 = (231,47,42)
dgreen = tuple(ti/255 for ti in g1)
lgreen = tuple(ti/255 for ti in g2)
pink = tuple(ti/255 for ti in p1)
red = tuple(ti/255 for ti in r1)
...
pt = pm1 & pm2 & pm3
pinknorm = (cy3norm * pt).clip(0, 255).astype(np.uint8)
r = red(cy3norm,0.5)
...
# plt.imshow(cy3norm*np.invert(pt), cmap = red)
plt.imshow(cy3norm, cmap = red)
...
plt.figure(frameon=False)
plt.imshow(pinknorm, cmap = pink)
plt.savefig('pinkonly.svg',format='svg',dpi=150,pad_inches=0)
plt.savefig('pinkonly.png',format='png',dpi=150,pad_inches=0)
...
im4 = plt.imshow(pinknorm, cmap=pink,extent=extent)
