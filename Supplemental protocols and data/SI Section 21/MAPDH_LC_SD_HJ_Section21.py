...
filename = 'SDPatterning_200umGels'
filename_W = 'SDPatterning_200umGels_wash'
exposure = (0, 100, 10) #exposure in ms
...
for i in range(0,len(xy_up)):   
    ...
    valve_timer(valves[2], 60) #flows buffer 60 seconds
    ...
    for i in range(len(fluorophore)):
        ...
        cv2.imwrite(filename + '_' + str(fluorophore[i])+'.tif', pixels.astype(np.uint16) )
        core.setProperty('DTOL-DAC-0', 'Volts', 0)
        time.sleep(2)
...
valve_timer(valves[3], 60) #flows wash solution for 60 seconds after patterning is over.